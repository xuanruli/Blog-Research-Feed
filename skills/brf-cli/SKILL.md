...
```json
{
  ...
  "title": "...",
  "url": "https://...",
  "published": "2026-05-19T14:23:00+00:00",  // may be null
  "summary": "≤500 chars",
...
- `x`: `summary` is the full tweet (retweets/quotes resolved to full referenced text); `has_full=true`, no extra drill needed.
- `firecrawl_index`: `summary` is empty by design (extracted from index pages) — triage on title + source. `published` comes from the index page's per-article date.
...
## Reading index.json (it's large — 300+ items)

`read` is capped per call → read in chunks with offset, OR project with one jq pass then read:
...
```bash
brf fetch x-user --handle HANDLE --since YYYY-MM-DD      # an X account not in feeds.yaml
