...
**v0 → v1 重构**：从 7 个 custom tool + cron runner dispatch 改成 CLI +
...
3. 打印 agent_id + env_id 到 stdout —— **这俩 ID 你不用动**，cron runner
...
改 environment.yaml（加包等）会自动建新 env —— cron runner 下次跑就用新的，
不需要更新任何 secret。
...
- `FIRECRAWL_API_KEY` · `X_BEARER_TOKEN` · `OPENAI_API_KEY` ·
  `SLACK_WEBHOOK_URL` — 这 4 个 cron runner 在每次 run 时打包成 `.env`
...
- 2026-05-19 v1：架构重构 — 删除 7 个 custom tool 和 cron runner dispatch
