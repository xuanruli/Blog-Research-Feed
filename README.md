...
| `cron/daily.py` (host) | Cron 端编排器：build .env、Files API upload、开 session、流式日志 |
...
3. 打印 agent_id + env_id 到 stdout —— **这俩 ID 你不用动**，cron runner 每次启动时按 `name`（来自 `agent/agent.yaml` / `environment.yaml`）到 Anthropic API 查实时 ID

需要改 agent 配置（system prompt / tools）时重跑 `python scripts/create_agent.py --update`（创建新版本）。改 `environment.yaml`（加包 / bump brf 版本）时：bump `name:` 到下一个 `v{N+1}` 然后重跑 `--update`，会自动建新 env。cron runner 下次跑按新 name 查表，**不需要更新任何 secret**。

### 3. 配 GitHub Secrets

仓库 Settings → Secrets → Actions，加 **5 个**：
- `ANTHROPIC_API_KEY` — cron runner 用来调 sessions/files API
- `FIRECRAWL_API_KEY` · `X_BEARER_TOKEN` · `OPENAI_API_KEY` · `SLACK_WEBHOOK_URL` — 这 4 个 cron runner 在每次 run 时打包成 `.env` 上传给容器；agent 在容器里 `brf` 自动读 `/workspace/.env`
...
- **2026-05-19 v0**：scaffold 完整管线 — 7 个 custom tool + cron-runner dispatch
