"""Upload (or version-bump) each custom skill under ``skills/`` to the workspace."""
...
def upload_one(client: Anthropic, skill_dir: Path, existing: dict[str, str]) -> str:
    """Create the skill, or add a new version if its display_title already exists."""
    ...
    if title in existing:
        ...
        client.beta.skills.versions.create(skill_id, files=files, betas=[SKILLS_BETA])
        print(f"# updated '{title}' -> {skill_id} (new version)", file=sys.stderr)
        return skill_id

    resp = client.beta.skills.create(display_title=title, files=files, betas=[SKILLS_BETA])
...
def main(argv: list[str]) -> int:
    only = set(argv[1:])
    dirs = [d for d in sorted(SKILLS_DIR.iterdir()) if d.is_dir() and (not only or d.name in only)]
    if not dirs:
